...
# Global safety error handler wrapping around the interface initialization
try:
    ...
    # --- LEVEL D: ACTIVE RUNTIME APPLICATION WORKSPACE ---
        ...
        if st.session_state.generating and streaming_target_placeholder is not None:
            ...
            # We wrap the entire backend resolution AND the initial stream preparation 
            # inside this spinner block so the UI shows active progress to the coach.
            with st.spinner("Stepping up to the plate... Analyzing playbook and calculating strategy..."):
                ...
                # Pre-fetch the stream iterator inside the spinner so the API call is covered
                chunks_iterator = get_stream_chunks()

            # Stream directly into the persistent layout slot managed by Section 5
            with streaming_target_placeholder.container():
                with st.chat_message("assistant", avatar="🥎"):
                    full_response = st.write_stream(chunks_iterator)
                    if sources:
                        with st.expander("📚 Sources used"):
                            for s in sources:
                                st.caption(f"• {s}")
            ...
            # Force an instantaneous structural rerun.
            st.rerun()

except Exception as e:
    st.error(" An unhandled exception structural crash occurred:")
    st.exception(e)