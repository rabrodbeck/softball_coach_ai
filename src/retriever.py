...
7. CUSTOM ON-BASE PERCENTAGE (OBP) FORMULA:
   - The team uses a customized OBP formula that includes Reached on Error (ROE):
     OBP = (Singles + Doubles + Triples + HR + BB + HBP + ROE) / (AB + BB + HBP + ROE)
   - Note that Hits (H) is the sum of Singles + Doubles + Triples + HR. So the numerator is also (Hits + BB + HBP + ROE).
   - Whenever you calculate, compare, or explain a player's OBP, ensure you use this custom formula instead of the standard baseball OBP formula."""
