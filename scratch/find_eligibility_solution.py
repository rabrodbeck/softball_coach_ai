...
# Let's try adding positions
print("Testing single position additions to find a solution:")
for i, p in enumerate(base_players):
    all_possible_positions = ["P", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"]
    missing_positions = [pos for pos in all_possible_positions if pos not in p["eligible_positions"]]
    for pos in missing_positions:
        # Clone players and add eligibility
        temp_players = [dict(x) for x in base_players]
        temp_players[i]["eligible_positions"] = base_players[i]["eligible_positions"] + [pos]
        if test_config(temp_players):
            print(f" -> Success! Adding {pos} to {p['name']}")
